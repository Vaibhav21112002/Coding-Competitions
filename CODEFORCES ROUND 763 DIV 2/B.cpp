...
void helperFunction(int st, int en){
    if(st == en){
        ans[{st,en}] = st;
        return;
    }
    for(int i= st; i<= en; i++){
        if( i == en){
            if(m[{st,i-1}]){
                ans[make_pair(st,en)] = i;
                helperFunction(st,i-1);
                break;
            }
        }else if(i == st){
            if(m[{i+1,en}]){
                ans[make_pair(st,en)] = i;
                helperFunction(i+1,en);
                break;
            }
        }
        else{
            if(m[{st,i-1}] and m[{i+1,en}]){
                ans[make_pair(st,en)] = i;
                helperFunction(i+1,en);
                helperFunction(st,i-1);
                break;
...
void solve(int tt)
{
    ...
    for(auto i:m){
        if(ans[i.ff]){
            cout<< i.ff.ff << " " << i.ff.ss << " " << ans[i.ff] << endl;
        }
    }
    cout << endl;
}   
